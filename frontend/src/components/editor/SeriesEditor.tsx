...
export const SeriesEditor: React.FC<SeriesEditorProps> = ({ pdfUrl }) => {
  ...
  const handleAddMasterSlot = useCallback(() => {
    if (masterSlot) {
      safeToast('Only one master slot allowed');
      return;
    }
  ...
  const handleGenerateOutput = useCallback(() => {
    if (!masterSlot) {
      safeToast('Add a master slot first');
      return;
    }
    
    if (!startingSeries || !/\d+$/.test(startingSeries)) {
      safeToast('Starting series must end with numbers (e.g., A001)');
      return;
    }
  ...
  const handleShowPreview = useCallback(() => {
    if (outputPages.length === 0) {
      safeToast('Generate output first');
      return;
    }
    setShowPreview(true);
  }, [outputPages]);
