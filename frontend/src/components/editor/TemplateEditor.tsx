...
export const TemplateEditor: React.FC<TemplateEditorProps> = ({ pdfCanvasRef, pdfDimensions }) => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    newX = Math.max(0, Math.min(newX, EDITOR_WIDTH - 80));
    newY = Math.max(0, Math.min(newY, EDITOR_HEIGHT - 24));
    
    setInputBlocks(blocks => 
      blocks.map(b => b.id === selectedBlockId ? { ...b, x: newX, y: newY } : b)
    );
  }, [isDragging, selectedBlockId, dragOffset]);
  ...
  const handleGenerateOutput = () => {
    if (inputBlocks.length === 0) {
      safeToast('Add at least one series input.');
      return;
    }
    
    const primaryBlock = inputBlocks.find(b => b.value && /\d+$/.test(b.value));
    if (!primaryBlock) {
      safeToast('Enter a series value with numbers.');
      return;
    }

    const seriesValues = parseSeriesValues();
    if (seriesValues.length === 0) {
      safeToast('No series values to generate.');
      return;
    }
  ...
  const handlePrint = () => {
    ...
    if (!printWindow) {
      safeToast('Allow popups for printing.');
      return;
    }
    ...
    <div className="flex h-full bg-background">
      ...
      <div className="w-[420px] border-r border-border p-4 flex flex-col gap-4">
        ...
        <div className="rounded-lg border border-border bg-card p-3">
          ...
          <div
            ref={editorRef}
            className="relative rounded border-2 border-dashed border-primary/40 bg-[#4a7cc9] overflow-hidden cursor-crosshair"
                  ...
                  >
                    <div className="flex flex-col h-full">
                      {page.slots.map((slot, slotIndex) => (
                        <div
                          key={slotIndex}
                          className="relative flex-1 border-b border-dashed border-gray-300 last:border-b-0"
                        >
