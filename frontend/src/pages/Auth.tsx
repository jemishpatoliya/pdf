...
const safeToast = (msg: unknown) => {
  try {
    toast(String(msg ?? 'Something went wrong'));
  } catch {
  }
};
...
const Auth = () => {
  ...
  const validateForm = () => {
    ...
    if (!emailResult.success) {
      ...
      newErrors.email = typeof errorMessage === 'string' ? errorMessage : String(errorMessage || 'Invalid email');
    }
    
    const passwordResult = passwordSchema.safeParse(password);
    if (!passwordResult.success) {
      const errorMessage = passwordResult.error.errors[0]?.message;
      newErrors.password = typeof errorMessage === 'string' ? errorMessage : String(errorMessage || 'Invalid password');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error, data } = await signIn(String(email || '').trim(), String(password || ''));
      
      if (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        if (String(errorMessage).includes('Invalid email or password')) {
          safeToast('Invalid email or password');
        } else {
          safeToast(errorMessage);
        }
        return;
      }

      if (!data?.user?.role) {
        safeToast('Login failed: missing user role');
        return;
      }

      toast.success('Welcome back!');

      navigate(data.user.role === 'ADMIN' ? '/upload' : '/printing', { replace: true });
    } catch (err) {
      const error = err instanceof Error ? err.message : String(err);
      safeToast(String(error) || 'An unexpected error occurred');
    } finally {
      setLoading(false);
    }
  };
