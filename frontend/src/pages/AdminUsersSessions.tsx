...
const AdminUsersSessions = () => {
  ...
  const handleChangePassword = async () => {
    if (!currentPassword || !newPassword) {
      safeToast("Current password and new password are required");
      return;
    }
    
    if (newPassword.length < 6) {
      safeToast("New password must be at least 6 characters long");
      return;
    }
  ...
  const handleCreateAdmin = async () => {
    ...
    if (adminPassword.length < 6) {
      safeToast("Password must be at least 6 characters long");
      return;
    }
